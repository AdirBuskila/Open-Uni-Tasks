    ...
    <div className="App">
      ...
      <ul>
        {assignments.map((assignment, index) => (
          <li
            key={index}
            className={listItemStyle(assignment)}
            style={{ backgroundColor: assignment.color }}
          >
            {assignment.courseIcon} {assignment.course}: {assignment.name}{" "}
            {assignmentIcon(assignment.name)} - Due on {assignment.dueDate} -{" "}
            {daysUntil(assignment.dueDate) < 0
              ? "Submit time over"
              : `${daysUntil(assignment.dueDate)} day(s) left`}
          </li>
        ))}
      </ul>
    </div>
